  ...
  template: `
    <div class="h-full w-full relative overflow-hidden flex flex-col p-4 bg-gray-900 animate-fadeIn">
      ...
      <header class="relative z-10 text-center mb-4">
        ...
        <h1 class="text-4xl font-orbitron text-red-300 drop-shadow-lg">Artisan d'Armes</h1>
      </header>
       ...
       <main class="relative z-10 flex-grow grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto pr-2">
        <div class="bg-black/30 p-4 rounded-xl border border-gray-700/50 flex flex-col">
          ...
          <div class="flex-grow overflow-y-auto space-y-2">
            ...
            @if(!hasMaterials()) {
              <p class="text-center text-gray-500 italic py-8">You have no weaponcrafting materials.</p>
            }
          </div>
        </div>
        <div class="bg-black/30 p-6 rounded-xl border border-gray-700/50 flex flex-col items-center justify-center text-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-24 w-24 mx-auto text-gray-600 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1"><path stroke-linecap="round" stroke-linejoin="round" d="M21 3l-6 6m0 0l-1.5 1.5M15 9l1.5-1.5M3 21l6-6m0 0l1.5 1.5M9 15l-1.5 1.5m6-1.5l-1.5-1.5M9 9l1.5-1.5" /></svg>
          <h2 class="text-2xl font-orbitron text-gray-500">Weaponsmith's Bench</h2>
          <p class="text-gray-400 mt-2">Schematics for crafting powerful bows, crossbows, and staves will be available here soon.</p>
        </div>
      </main>
    </div>
  `,
...
export class ArtisanArmesComponent {
  ...
  materials = computed(() => {
    ...
    return ALL_MATERIALS
      .filter(m => m.type === 'Ore' || m.type === 'Wood')
      .map(material => ({
        ...material,
