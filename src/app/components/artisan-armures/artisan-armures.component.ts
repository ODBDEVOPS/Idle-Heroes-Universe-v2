...
@Component({
  selector: 'app-artisan-armures',
  standalone: true,
  template: `
    <div class="h-full w-full relative overflow-hidden flex flex-col p-4 bg-gray-900 animate-fadeIn">
      <div class="absolute inset-0 bg-gradient-to-b from-gray-900 via-gray-700/40 to-gray-900 opacity-50"></div>
      ...
      <header class="relative z-10 text-center mb-4">
        ...
        <h1 class="text-4xl font-orbitron text-gray-300 drop-shadow-lg">Artisan d'Armures</h1>
      </header>
      ...
      <main class="relative z-10 flex-grow grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto pr-2">
        <div class="bg-black/30 p-4 rounded-xl border border-gray-700/50 flex flex-col">
          ...
          <div class="flex-grow overflow-y-auto space-y-2">
            ...
            @if(!hasMaterials()) {
              <p class="text-center text-gray-500 italic py-8">You have no armorsmithing materials.</p>
            }
          </div>
        </div>
        <div class="bg-black/30 p-6 rounded-xl border border-gray-700/50 flex flex-col items-center justify-center text-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-24 w-24 mx-auto text-gray-600 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1"><path stroke-linecap="round" stroke-linejoin="round" d="M9 20V12L5 12V8C5 5.79086 6.79086 4 9 4H15C17.2091 4 19 5.79086 19 8V12L15 12V20H9Z" /></svg>
          <h2 class="text-2xl font-orbitron text-gray-500">Armorer's Workbench</h2>
          <p class="text-gray-400 mt-2">Patterns for forging specialized plate and mail armors will be available here soon.</p>
        </div>
      </main>
    </div>
  `,
...
export class ArtisanArmuresComponent {
  ...
  materials = computed(() => {
    ...
    return ALL_MATERIALS
      .filter(m => m.type === 'Ore' || m.type === 'Leather')
      .map(material => ({
        ...material,
