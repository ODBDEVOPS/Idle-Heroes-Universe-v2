...
import { ALL_MATERIALS } from '../../app/models/material.model';
// FIX: Changed import for Rarity from hero.model to equipment.model
import { Rarity } from '../../app/models/equipment.model';
  ...
  template: `
    <div class="h-full w-full relative overflow-hidden flex flex-col p-4 bg-gray-900 animate-fadeIn">
      ...
      <header class="relative z-10 text-center mb-4">
        ...
        <h1 class="text-4xl font-orbitron text-stone-300 drop-shadow-lg">Architecte</h1>
      </header>
       ...
       <main class="relative z-10 flex-grow grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto pr-2">
        <div class="bg-black/30 p-4 rounded-xl border border-gray-700/50 flex flex-col">
          ...
          <div class="flex-grow overflow-y-auto space-y-2">
            ...
            @if(!hasMaterials()) {
              <p class="text-center text-gray-500 italic py-8">You have no architectural materials.</p>
            }
          </div>
        </div>
        <div class="bg-black/30 p-6 rounded-xl border border-gray-700/50 flex flex-col items-center justify-center text-center">
          ...
          <p class="text-gray-400 mt-2">Blueprints for new buildings and guild improvements will be available here soon.</p>
        </div>
      </main>
    </div>
  `,
...
export class ArchitecteComponent {
  ...
  materials = computed(() => {
    ...
    return ALL_MATERIALS
      .filter(m => m.type === 'Wood' || m.type === 'Stone')
      .map(material => ({
        ...material,
