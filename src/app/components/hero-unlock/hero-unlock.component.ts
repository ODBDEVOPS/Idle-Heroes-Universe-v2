...
@Component({
  selector: 'app-hero-unlock',
  standalone: true,
  templateUrl: './hero-unlock.component.html',
  imports: [CommonModule, TooltipDirective],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class HeroUnlockComponent {
  ...
  hero = computed(() => {
    ...
    return this.gameService().heroes().find(h => h.id === id) ?? null;
  });

  unlockCost = computed(() => {
    const h = this.hero();
    if (!h || h.level > 0) return 0;
    return h.nextLevelCost;
  });

  unlockHero() {
    const h = this.hero();
    if (h && h.level === 0 && this.gameService().gameState().gold >= this.unlockCost()) {
      this.gameService().levelUpHero(h.id);
      // Navigate to the team management screen so the user can use their new hero
