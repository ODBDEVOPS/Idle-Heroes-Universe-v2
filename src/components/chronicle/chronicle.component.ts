...
export class ChronicleComponent implements OnInit {
  ...
  canWeaveMemory = computed(() => this.gameService().gameState().prestigePoints >= this.WEAVE_MEMORY_COST);
  ...
  async forgeDestiny(memoryId: string) {
    const hero = this.selectedHero();
    if (!hero || this.isForgingQuest() || this.gameService().gameState().gold < this.FORGE_DESTINY_COST) return;
