...
export class TeamComponent implements OnDestroy {
  ...
  // Health animations
  healthUpdateAnimation = signal<{ [heroId: number]: 'damage' | 'heal' }>({});
  private animationTimeouts: { [heroId: number]: any } = {};
  ...
  constructor() {
    effect(() => {
      const update = this.gameService().heroHealthUpdate();
      if (update) {
        const { heroId, type } = update;

        // Clear any existing timeout for this hero to restart the animation
        if (this.animationTimeouts[heroId]) {
          clearTimeout(this.animationTimeouts[heroId]);
        }

        this.healthUpdateAnimation.update(animations => ({
          ...animations,
          [heroId]: type
        }));
        
        this.animationTimeouts[heroId] = setTimeout(() => {
          this.healthUpdateAnimation.update(animations => {
            const newAnims = { ...animations };
            delete newAnims[heroId];
            return newAnims;
          });
          delete this.animationTimeouts[heroId];
  ...
  ngOnDestroy() {
    // Clear all running timeouts when the component is destroyed
    for (const heroId in this.animationTimeouts) {
      clearTimeout(this.animationTimeouts[heroId]);
    }
  }
  ...
  filteredAndSortedHeroes = computed<DisplayHero[]>(() => {
    ...
    const sortedHeroes = heroes.slice().sort((a, b) => {
        // Unlocked heroes always come before locked heroes
        if (a.level > 0 && b.level === 0) return -1;
        if (a.level === 0 && b.level > 0) return 1;

        // Favorited heroes always come first within their group (locked/unlocked)
        ...
        switch (sort) {
            ...
            case 'favorite': compare = 0; break; // Favorite is already handled above
        }

        // For name sort, direction can be asc/desc. For others, it's desc only.
        if (sort === 'name') {
